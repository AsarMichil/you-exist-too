<script lang="ts">
	...
	// Generate random delay for lightbulb animations
...
<div class="flex flex-col w-full">
	{#if thoughts.length > 0}
		<div
			class="flex justify-between items-center p-3 border-b border-slate-700 dark:border-white mb-6"
		>
			<div class="flex items-center">
				<button
					class="flex items-center gap-1 py-1 px-2 rounded hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors text-sm"
		...
		{#if totalPages > 1}
			<div class="pagination-bottom flex justify-center items-center gap-4 mt-6">
				<button
					class="flex items-center gap-1 px-3 py-1 border border-slate-700 dark:border-white rounded hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors disabled:opacity-50 disabled:cursor-not-allowed text-sm"
					onclick={prevPage}
					disabled={currentPage === 1}
					aria-label="Previous page"
				>
	...
	{:else}
		<EmptyState
			...
			message="No one has thought about you yet. Share your profile with friends to see thoughts appear here as beautiful lightbulbs."
		/>
	{/if}
</div>
