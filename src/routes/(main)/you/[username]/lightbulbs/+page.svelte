<script lang="ts">
	...
	import LightbulbsContainer from '$lib/components/LightbulbsContainer.svelte';
	...
	function goToStringLights() {
		goto(`/you/${data.props?.person?.username}/stringlights`);
	}
</script>

<svelte:head>
	<title
		>{data.props?.person?.preferred_name ?? data.props?.person?.given_name} | Lightbulbs | You Exist Too</title
	>
</svelte:head>
		...
		<div class="flex flex-col items-center w-full">
			<div class="w-full">
				<h2 class="text-2xl mb-4 text-center">
					{thoughts ? 'These people have thought about you (Lightbulb Edition)' : 'Who has thought about you?'}
				</h2>
					...
					{#if !thoughts}
						<div class="text-center mb-6">
							<p class="text-slate-600 dark:text-slate-300 mb-4">
								View who has been thinking about you, represented as glowing lightbulbs. Each lightbulb
							...
							<form method="post" action={'?/getThoughts'}>
								<Button type="submit" class="w-48">
									{form?.openLightbulbs ? 'Hide Thoughts' : 'View Thoughts'}
								</Button>
							</form>
						</div>
					{:else}
						<div class="mb-4 flex justify-between">
							...
							<button 
								class="text-sm text-slate-600 dark:text-slate-300 hover:text-forestgreen-600 dark:hover:text-forestgreen-400 transition-colors"
								onclick={goToStringLights}
							>
								Try String Lights View →
							</button>
						</div>
						<LightbulbsContainer thoughts={thoughts || []} />
					{/if}
				</div>
			</div>
		</div>
