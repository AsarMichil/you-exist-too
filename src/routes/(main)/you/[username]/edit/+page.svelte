<script lang="ts">
	...
	const {
		...
		enhance: socialLinksEnhance,
		submitting: socialLinksSubmitting
	} = superForm(data.socialLinksForm);
...
<Dialog.Root bind:open>
	<Dialog.Portal>
		...
		>
			<div class="h-[90vh] overflow-y-auto flex flex-col px-1">
				...
				<form method="POST" use:superEnhance action="?/update_profile" class="">
					<div class="mb-4">
				...
				<form method="POST" use:socialLinksEnhance action="?/social_links">
					<h2 class="text-lg font-mont">Social Links</h2>
					<div class="mb-4">
						<label for="instagram" class="block mb-2 font-medium">Instagram Username</label>
						<Input id="instagram" name="instagram" bind:value={$socialLinksForm.instagram} />
						{#if $socialLinksErrors.instagram}
							<p class="text-red-500 mt-1">{$socialLinksErrors.instagram}</p>
						{/if}
					</div>
					<button
						type="submit"
						disabled={$socialLinksSubmitting}
						class="border-slate-800 border-2 rounded-md py-2 px-3 hover:bg-forestgreen-400 active:bg-forestgreen-700 dark:hover:bg-forestgreen-400 dark:active:bg-forestgreen-700 dark:border-white dark:focus:border-forestgreen-700 outline-none focus:border-forestgreen-700 focus:ring-2 focus:ring-forestgreen-700 disabled:opacity-50"
					>
						{$socialLinksSubmitting ? 'Saving...' : 'Save Changes'}
					</button>
				</form>
				<hr class="my-4 border-slate-800 dark:border-white" />
				<div class="w-fit">
